...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initializeResources() {
        try {
            ...
            if (expertRepository.count() == 0) {
                List<Expert> experts = loadResource(resolveDataPath(baseDir, "res_experts.json"), new TypeReference<List<Expert>>() {}, mapper);
                if (experts != null) expertRepository.saveAll(experts);
            }

            // Policies
            if (policyRepository.count() == 0) {
                List<Policy> policies = loadResource(resolveDataPath(baseDir, "res_policies.json"), new TypeReference<List<Policy>>() {}, mapper);
                if (policies != null) policyRepository.saveAll(policies);
            }

            // Funds
            if (fundRepository.count() == 0) {
                List<Fund> funds = loadResource(resolveDataPath(baseDir, "res_funds.json"), new TypeReference<List<Fund>>() {}, mapper);
                if (funds != null) fundRepository.saveAll(funds);
            }

            // Equipments
            if (equipmentRepository.count() == 0) {
                List<Equipment> equipments = loadResource(resolveDataPath(baseDir, "res_equipments.json"), new TypeReference<List<Equipment>>() {}, mapper);
                if (equipments != null) equipmentRepository.saveAll(equipments);
            }

            // Patents
            if (patentRepository.count() == 0) {
                List<Patent> patents = loadResource(resolveDataPath(baseDir, "res_patents.json"), new TypeReference<List<Patent>>() {}, mapper);
                if (patents != null) patentRepository.saveAll(patents);
            }

            // Enterprises
            if (enterpriseRepository.count() == 0) {
                List<Enterprise> enterprises = loadResource(resolveDataPath(baseDir, "res_enterprises.json"), new TypeReference<List<Enterprise>>() {}, mapper);
    ...
    private String getDataDir() {
        return System.getenv().getOrDefault("DATA_DIR", "data");
    }

    private String resolveDataPath(String dataDir, String fileName) {
    ...
    private void initializeAchievements() {
        try {
            ...
            mapper.registerModule(new JavaTimeModule());
            mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
            File file = new File(resolveDataPath(getDataDir(), "smart_achievements.json"));
    ...
    private void initializeDemands() {
        try {
            ...
            File file = new File(resolveDataPath(getDataDir(), "smart_demands.json"));
